...
from core.utils.network_util import sample_from_planes
...
class BlendNet(nn.Module):
    def __init__(
        ...
        self.pix_linear = nn.Sequential(
            ...
            # nn.Linear(featmaps_n_channels+2*pos_embed_xyz_dim+pos_embed_dir_dim+2+mweights_n_channels, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
        )
        ...
        #     nn.Linear(3*32+2*pos_embed_xyz_dim+pos_embed_dir_dim+1+mweights_n_channels, 128),
        #     nn.ReLU(),
        #     nn.Linear(128, 64),
        # )
        self.glob_vol_linear = nn.Sequential(
            nn.Linear(3*32+feat_vol_n_channels+2*pos_embed_xyz_dim+pos_embed_dir_dim+1+mweights_n_channels, 256),
            # nn.Linear(3*32+2*pos_embed_xyz_dim+1+mweights_n_channels, 128),
            nn.ReLU(),
            nn.Linear(256, 128),
        )
        self.query_linear = nn.Sequential(
            nn.Linear(2*pos_embed_xyz_dim+pos_embed_dir_dim+1+mweights_n_channels, 128),
            # nn.Linear(2*pos_embed_xyz_dim+1+mweights_n_channels, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
        )
    ...
    def forward(
        ...
        **_,
        ...
        global_feat = sample_from_planes(
            global_feature_planes, 
            cnl_pts_norm,
        )
        glob_vol_input = torch.cat((global_feat, point_feat, query_pts_with_view_dir), dim=-1)
        glob_vol_feat = self.glob_vol_linear(glob_vol_input)
        glob_feat, vol_feat = torch.chunk(glob_vol_feat, 2, dim=-1)

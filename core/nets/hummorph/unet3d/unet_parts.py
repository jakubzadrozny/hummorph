# adapted from: https://github.com/szymanowiczs/viewset-diffusion/blob/main/model/unet_parts.py
...
#         super().__init__()
#         self.conv = nn.Conv3d(dim * 8, default(dim_out, dim), 1)

#     def forward(self, x):
#         # D, H, W = x.shape[-3:]
#         # padding = (0, W % 2, 0, H % 2, 0, D % 2)
#         # if sum(padding) > 0:
#         #     x = F.pad(x, padding)
#         x = rearrange(x, 'b c (h p1) (w p2) (d p3) -> b (c p1 p2 p3) h w d', p1 = 2, p2 = 2, p3=2)
#         return self.conv(x)

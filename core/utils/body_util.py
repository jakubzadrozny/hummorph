...
def _deform_gaussian_volume(
    ...
    zgrid, ygrid, xgrid = np.meshgrid(
        ...
        # np.arange(min_z, max_z+vsize, vsize),
        # np.arange(min_y, max_y+vsize, vsize),
        # np.arange(min_x, max_x+vsize, vsize),
        indexing='ij')
...
def approx_gaussian_bone_volumes(
    ...
    grid_shape = [grid_size//2, grid_size, grid_size]
    # vsize = 0.075
    # vsize = np.max((bbox_max_xyz - bbox_min_xyz)) / (grid_size - 1) + 1e-6
    # grid_shape = list(
    #     np.ceil((bbox_max_xyz - bbox_min_xyz) / vsize).astype(int) + 1
    # )[::-1]
    ...
    g_volumes = np.concatenate([g_volumes, bg_volume], axis=0) # + 1e-6
    g_volumes = g_volumes / np.sum(g_volumes, axis=0, keepdims=True).clip(min=0.001)
    
    return g_volumes


def get_mweights_dist_penalty(canonical_joints, pts):
    dist = np.sum((canonical_joints[:, None, :] - pts[None, ...])**2, axis=-1)

    for joint_idx, parent_idx in SMPL_PARENT.items():
        heads = canonical_joints[joint_idx]
        tails = canonical_joints[parent_idx]

        t = _dot_np(pts - heads, tails - heads) / _dot_np(tails - heads, tails - heads)
        p = heads + (tails - heads) * np.clip(t, 0, 1)
        bone_dist = np.sum((p - pts)**2, axis=-1)
        dist[parent_idx] = np.minimum(dist[parent_idx], bone_dist)
...
def closest_distance_to_points(
    joints, points, min_index=None, individual: bool = False
):
    ...
    ret = (points - joints[p_index]).clone().detach().reshape(points.shape[0], 3)
    ret = ret.to(torch.float32)
    return ret, min_index


def plot_pose(frame, joints_2d, alpha=0.5):
    pose_img = frame.copy()
    for idx, pos in enumerate(joints_2d):
        pose_img = cv2.circle(pose_img, pos, 3, (255, 0, 0), -1)
        parent = SMPL_PARENT.get(idx, None)
        if parent is not None:
            parent_pos = joints_2d[parent]
            pose_img = cv2.line(pose_img, pos, parent_pos, (255, 0, 0))
    frame = cv2.addWeighted(pose_img, alpha, frame, 1 - alpha, 0)
    return frame


def plot_pose_only(frame, joints_2d):
    H, W = frame.shape[:2]
    pose_img = np.zeros((2*H, 2*W, 4), dtype=np.uint8)
    for idx, pos in enumerate(joints_2d):
        pos = 2*pos
        pose_img = cv2.circle(pose_img, pos, 5, (0, 0, 255, 255), -1)
        parent = SMPL_PARENT.get(idx, None)
        if parent is not None:
            parent_pos = 2*joints_2d[parent]
            pose_img = cv2.line(pose_img, pos, parent_pos, (0, 0, 255, 255), 2)
    return pose_img
